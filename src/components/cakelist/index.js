...
class Cakelist extends Component{
    ...
    init(page){
        $.get('http://localhost:2323/goodslist', {goodsid: 'cake', page: page || 1}, res => {
            this.state.pageNum = []
    ...
    queryBtn(){
        $.get('http://localhost:2323/goodslist', {goodsid: 'cake',name:$('.goodsName').val()}, res => { 
            if(res.code === 1) {
                this.setState({
                    data: res.data
                })
